...
int main()
{
    ...
    {
        cppl::GzipFile writer = cppl::GzipFile::openForAppend(filename);
        if (writer.valid())
        {
...

    {
        ...
        cppl::GzipFile reader = cppl::GzipFile::openForRead(filename);
        if (reader.valid())
        {
            char buf[256];
    ...
    {
        cppl::GzipFile writer = cppl::GzipFile::openForWriteExclusive(filename);
